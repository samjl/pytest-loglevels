...
# @created 12/07/17
# @brief pytest plugin loglevels - functions to assign and print a log level to test log messages.
# Current limitations: log level cannot be applied to standard print function.
...
def pytest_namespace():
    ...
    class LogLevel:
        """Class containing logging methods used to apply a log level to a message
        ...
        def detail_step(msg):
            ...
            set_log_parameters("Detail level step: {}".format(msg), log_level=2)

        @staticmethod
        def step(msg, log_level=None):
            """Print a message at the specified or current log level.
            If optional argument log_level is not specified or None then the log level of the previous message is applied.
            """
            set_log_parameters("Step: {}".format(msg), log_level)

        @staticmethod
        def step_increment(msg, increment=1):
            """Increment the current log level and print message at the new level."""
            current_level = MultiLevelLogging.current_level
            set_log_parameters("Step inc: {}".format(msg), current_level + increment)
...
def set_log_parameters(msg, log_level):
    """Prepend the string to print with the log level and step before printing"""
...
class MultiLevelLogging:
    # Keep track of the current log level and the step for each log level
    current_level = 1
...
def reset_level_step(log_level):
    # Reset the step for a log level to 0. Next time this level is logged hte step will be set to 1.
